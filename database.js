var cacheName = 'tiny_notes'; //changing this version will invalidate the cached app shell
const version = 1;
...
var db;
var request = indexedDB.open(cacheName, version);
...
request.onupgradeneeded = function(event) {
  ...
  var objectStore = event.target.result.createObjectStore("notes");
...
function saveRecord(id, json, callback){
  var tableName = 'notes'
  if(db == undefined){
...
function readRecord(id, callback){
  if(db !== undefined){
    var transaction = db.transaction('notes', "readwrite")
    var table = transaction.objectStore('notes');
    var request = table.get(id);
